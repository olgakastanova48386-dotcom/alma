...
export default function UnifiedMap() {
  ...
  return <section className="bg-[#f7f4ef] pb-10 text-black">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-5 rounded-[26px]">
        <div className="flex items-end justify-between gap-4 px-1 pb-2 pt-1">
        ...
        <div className="flex snap-x snap-mandatory items-start gap-3 overflow-x-auto px-0 pb-1 pt-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {filtered.map((p) => <article key={p.id} className={`w-[82vw] max-w-[330px] shrink-0 self-start snap-center overflow-hidden rounded-[22px] border transition-all duration-300 ${selected === p.id ? "border-black bg-black text-white shadow-lg" : "border-black/5 bg-[#faf8f5] hover:border-black/15"}`}>
              <button onClick={() => selected === p.id ? router.push(p.detailHref) : focus(p)} className="w-full text-left">
                ...
                <div className="p-3.5 sm:p-4">
                  ...
                  {p.safePlace && <div className="mt-2.5"><span className={`inline-flex rounded-full bg-[#f4d9df] px-2.5 py-1 text-[11px] font-semibold text-[#6f2437]`}>🛡 Безопасное место</span></div>}
