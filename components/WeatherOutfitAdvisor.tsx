...
export default function WeatherOutfitAdvisor() {
  ...
  return (
    <div className="fixed bottom-[calc(94px+env(safe-area-inset-bottom))] right-3 z-[70] sm:bottom-5 sm:left-6 sm:right-auto">
      ...
      <button type="button" onClick={() => { if (weather) { setOpen(true); return; } requestLocation(true); }} className="flex h-11 w-11 items-center justify-center rounded-full border border-black/10 bg-black text-[18px] font-semibold text-white shadow-xl transition hover:scale-[1.02] hover:bg-neutral-800 sm:h-auto sm:w-auto sm:px-5 sm:py-3 sm:text-sm" aria-label="Что надеть по погоде?"><span aria-hidden="true">☁️</span><span className="hidden sm:inline"> Что надеть?</span></button>
    </div>
  );
}
