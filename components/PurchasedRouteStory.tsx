...
const exactOrContained=(table:Record<string,string>,name:string)=>{
  const normalized=normalizedName(name);
  if(table[normalized])return table[normalized];
  const hit=Object.entries(table).find(([key])=>normalized.includes(key));
  return hit?.[1];
};

const stayTime=(p:PurchasedRouteStop)=>{
  const verified=exactOrContained(routeStayTimes,p.name);
...
const story=(p:PurchasedRouteStop)=>exactOrContained(routeStories,p.name)??null;
...
function smartTransit(a:PurchasedRouteStop,b:PurchasedRouteStop):TransitPlan{
  ...
  if(!sameStation&&toFrom<=12&&fromTo<=12&&metroTotal<=walk-6)return{mode:"metro",minutes:metroTotal,title:`Лучше всего: метро · ≈ ${metroTotal} мин`,details:[`${toFrom} мин пешком → ${from.name}`,`${from.line}${sameLine?"":" · 1 пересадка"}`,`${to.name} → ${fromTo} мин пешком до следующей точки`]};
  const bus=busNumber(a,b);
  if(bus){const total=Math.max(18,Math.round(walk*0.55));return{mode:"bus",minutes:total,title:`Лучше всего: автобус №${bus} · ≈ ${total} мин`,details:["Без лишнего спуска в метро","ALMA выбирает наземный вариант, потому что здесь он удобнее пешего пути и метро."]};}
  if(!sameStation&&toFrom<=15&&fromTo<=15)return{mode:"metro",minutes:metroTotal,title:`Метро · ≈ ${metroTotal} мин`,details:[`${toFrom} мин пешком → ${from.name}`,`${from.line}${sameLine?"":" · с пересадкой"}`,`${to.name} → ${fromTo} мин пешком`]};
