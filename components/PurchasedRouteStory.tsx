...
export default function PurchasedRouteStory({stops,romantic=false,onReset}:{stops:PurchasedRouteStop[];romantic?:boolean;onReset:()=>void}){
 ...
 const [recipientPreview,setRecipientPreview]=useState(false);

 useEffect(()=>{
  try{
   const raw=localStorage.getItem(INVITE_DRAFT_KEY);
   if(!raw) return;
   const savedDraft=JSON.parse(raw) as {guestName?:string;date?:string;time?:string;meeting?:string;note?:string};
   if(savedDraft.guestName) setGuestName(savedDraft.guestName);
   if(savedDraft.date) setDate(savedDraft.date);
   if(savedDraft.time) setTime(savedDraft.time);
   if(savedDraft.meeting) setMeeting(savedDraft.meeting);
   if(savedDraft.note) setNote(savedDraft.note);
  }catch{}
 },[]);
 ...
 const shareInvite=async()=>{try{if(navigator.share){await navigator.share({title:"Приглашение от ALMA",text:inviteText});setShared(true);setTimeout(()=>setShared(false),1800)}else await copyInvite()}catch{}};
 ...
 const downloadCalendar=()=>{
  ...
  const ics=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//ALMA//Date Invitation//RU","BEGIN:VEVENT",`UID:alma-${Date.now()}@local`,`DTSTAMP:${fmt(new Date())}`,`DTSTART:${fmt(start)}`,`DTEND:${fmt(end)}`,`SUMMARY:${escape("Свидание с ALMA ✦")}`,`DESCRIPTION:${escape("Маршрут — сюрприз. Детали останутся скрыты до встречи.")}`,`LOCATION:${escape(meeting.trim()||"Санкт-Петербург")}`,"END:VEVENT","END:VCALENDAR"].join("\r\n");
  const blob=new Blob([ics],{type:"text/calendar;charset=utf-8"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");a.href=url;a.download="alma-date.ics";a.click();URL.revokeObjectURL(url);
 };
