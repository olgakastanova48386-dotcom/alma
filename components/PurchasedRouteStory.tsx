...
const story=(place:PurchasedRouteStop)=>verifiedStories[place.name.toLowerCase()]||"Для этой точки ALMA ещё проверяет исторический факт — лучше без красивой, но выдуманной легенды.";
...
export default function PurchasedRouteStory({stops,romantic=false,onReset}:{stops:PurchasedRouteStop[];romantic?:boolean;onReset:()=>void}){
  ...
  const routeText=useMemo(()=>{
    const lines=stops.map((place,i)=>{
      ...
      return `${i+1}. ${place.name}\n${place.category} · ${stayTime(place)}\n${place.address}\n${place.price}\n${place.description}\nЧто надеть: ${outfit(place.category)}${romantic?`\nМожно рассказать: ${story(place)}`:""}${walk}`;
    });
