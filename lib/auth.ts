...
export async function ensureAuthSchema() {
  ...
  await database.batch([
    database.prepare(
      `CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, name TEXT NOT NULL, phone TEXT NOT NULL UNIQUE, gender TEXT NOT NULL DEFAULT 'unspecified', password_hash TEXT NOT NULL, password_salt TEXT NOT NULL, marketing_sms INTEGER NOT NULL DEFAULT 0, phone_verified INTEGER NOT NULL DEFAULT 0, created_at INTEGER NOT NULL)`,
    ),
    database.prepare(
    ...
    database.prepare(
      `CREATE TABLE IF NOT EXISTS favorites (user_id TEXT NOT NULL, place_id INTEGER NOT NULL, created_at INTEGER NOT NULL, PRIMARY KEY(user_id, place_id), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`,
    ),
    database.prepare(
      `CREATE TABLE IF NOT EXISTS auth_rate_limits (scope TEXT NOT NULL, subject_hash TEXT NOT NULL, window_started_at INTEGER NOT NULL, hits INTEGER NOT NULL DEFAULT 0, PRIMARY KEY(scope, subject_hash))`,
    ),
    database.prepare(
      `CREATE TABLE IF NOT EXISTS email_codes (email TEXT NOT NULL, purpose TEXT NOT NULL, code_hash TEXT NOT NULL, expires_at INTEGER NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, created_at INTEGER NOT NULL, PRIMARY KEY(email, purpose))`,
    ),
    database.prepare(
    ...
    database.prepare(
      `CREATE TABLE IF NOT EXISTS photozone_submissions (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, name TEXT NOT NULL, address TEXT NOT NULL, note TEXT, mime_type TEXT NOT NULL, image_base64 TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending', created_at INTEGER NOT NULL, reviewed_at INTEGER, reviewed_by TEXT, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`,
    ),
    database.prepare(
    ...
    database.prepare(
      "CREATE INDEX IF NOT EXISTS idx_route_photos_place_status ON route_photos(place_id, status)",
    ),
    database.prepare(
      "CREATE INDEX IF NOT EXISTS idx_route_photos_status_created ON route_photos(status, created_at)",
    ),
    database.prepare(
      "CREATE INDEX IF NOT EXISTS idx_photozone_submissions_status_created ON photozone_submissions(status, created_at)",
    ),
    database.prepare(
...
export async function consumeAuthRateLimit(
  ...
  if (!row || now - Number(row.window_started_at) >= windowSeconds) {
    await db()
      .prepare(
        `INSERT INTO auth_rate_limits (scope, subject_hash, window_started_at, hits) VALUES (?, ?, ?, 1) ON CONFLICT(scope, subject_hash) DO UPDATE SET window_started_at = excluded.window_started_at, hits = 1`,
      )
  ...
  if (hits >= limit)
    return {
      ...
      retryAfter: Math.max(
        1,
        windowSeconds - (now - Number(row.window_started_at)),
      ),
    };
  await db()
    .prepare(
      "UPDATE auth_rate_limits SET hits = hits + 1 WHERE scope = ? AND subject_hash = ?",
    )
    .bind(scope, subjectHash)
    .run();
  return { allowed: true, retryAfter: 0 };
...
export async function issueEmailCode(
  ...
  const codeHash = await sha256(`${email}:${purpose}:${code}`);
  const now = Math.floor(Date.now() / 1000);
  await db()
    .prepare(
      `INSERT INTO email_codes (email, purpose, code_hash, expires_at, attempts, created_at) VALUES (?, ?, ?, ?, 0, ?) ON CONFLICT(email, purpose) DO UPDATE SET code_hash = excluded.code_hash, expires_at = excluded.expires_at, attempts = 0, created_at = excluded.created_at`,
    )
    .bind(email, purpose, codeHash, now + CODE_TTL, now)
    .run();
  return code;
}
export async function verifyEmailCode(
  ...
  if (!row || Number(row.expires_at) < now || Number(row.attempts) >= 5)
    return false;
  const candidate = await sha256(`${email}:${purpose}:${code}`);
  ...
  await db()
    .prepare("DELETE FROM email_codes WHERE email = ? AND purpose = ?")
    .bind(email, purpose)
    .run();
  return true;
}
...
export async function sendEmailCode(
  ...
  const from = String(
    (env as any).ALMA_EMAIL_FROM || "ALMA <onboarding@resend.dev>",
  ).trim();
  if (!apiKey) throw new Error("EMAIL_PROVIDER_NOT_CONFIGURED");
  const subject =
    purpose === "verify"
      ? "Код подтверждения ALMA"
      : "Восстановление доступа ALMA";
  const text = `${subject}: ${code}. Код действует 10 минут. Мы используем вашу почту только для подтверждения аккаунта и важных сообщений о доступе — никакого спама.`;
...
export async function createSession(userId: string) {
  ...
  await db()
    ...
    .bind(id, userId, now, now + SESSION_MAX_AGE)
    .run();
  return raw;
}
