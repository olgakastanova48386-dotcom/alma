...
const places = [
  ...
  {
    title: "Особняк Демидова",
    category: "Архитектура",
    image: "/images/demidov.jpg",
  },
  {
    title: "Академия Штиглица",
    category: "Искусство",
...
const photozones = [
  ...
  {
    title: "Ракета · Кожевенная линия, 27",
    tag: "Индустриальная эстетика",
    image: "/images/падл адрес ракета кожевенная линия, 27.jpg",
    time: "Днём или вечером · в помещении",
  },
  {
    title: "Новая Голландия",
    tag: "Городская эстетика",
    image: "/images/new-holland.jpg",
    time: "После 16:00",
  },
  {
...
function getHeroTheme(code?: number): HeroTheme {
  if (code === 0)
    return {
      image: "/images/питер главная фотка 4.jpg",
      position: "center 42%",
      label: "Ясный Петербург",
    };
  if ([1, 2].includes(code ?? -1))
    return {
      image: "/images/питер главная фотка.jpg",
      position: "center 42%",
      label: "Петербург в переменной облачности",
    };
  if (code === 3)
    return {
      image: "/images/питер главная фотка 2.jpg",
      position: "center 42%",
      label: "Пасмурный Петербург",
    };
  if ([45, 48].includes(code ?? -1))
    return {
      image: "/images/пиер главная фотка 3.jpg",
      position: "center 42%",
      label: "Петербург в тумане",
    };
  if ([51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82].includes(code ?? -1))
    return {
      image: "/images/питер главная фотка 5.jpg",
      position: "center 46%",
      label: "Петербург под дождём",
    };
  if ([71, 73, 75, 77, 85, 86].includes(code ?? -1))
    return {
      image: "/images/питер главная фотка 2.jpg",
      position: "center 42%",
      label: "Снежный Петербург",
    };
  if ([95, 96, 99].includes(code ?? -1))
    return {
      image: "/images/пиер главная фотка 3.jpg",
      position: "center 42%",
      label: "Драматичный Петербург",
    };
  return {
    image: "/images/питер главная фотка.jpg",
    position: "center 42%",
    label: "Петербург сегодня",
  };
}
...
export default function HomePage() {
  ...
  const scrollToFilters = () =>
    document
      .getElementById("alma-filters")
      ?.scrollIntoView({ behavior: "smooth", block: "center" });
  const scrollToPhotozones = () =>
    document
      .getElementById("alma-photozones")
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ef]">
      <section className="relative min-h-[620px] md:min-h-[820px] flex items-end md:items-center pt-28 sm:pt-28 pb-10 sm:pb-20 overflow-hidden bg-[#efe9e1] text-black">
        ...
        <div className="absolute z-10 bottom-8 right-8 hidden lg:flex items-center gap-3 text-xs uppercase tracking-[0.22em] text-neutral-500">
          <span className="w-8 h-px bg-black/20" />
          {heroTheme.label}
        </div>
      </section>

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-28">
        <div className="grid lg:grid-cols-[0.8fr_1.2fr] gap-7 sm:gap-10 lg:gap-20 items-end">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-neutral-500">
              ALMA · Санкт-Петербург
            </p>
            <h2 className="mt-4 sm:mt-5 text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.02]">
              Не просто
              <br />
              список мест
            </h2>
          </div>
          <p className="text-base sm:text-xl leading-7 sm:leading-8 text-neutral-500 max-w-2xl">
            Вместо бесконечного поиска выбирай, как ты хочешь провести время — а
            ALMA покажет подходящие места.
          </p>
        </div>
      </section>
      ...
      <section
        id="alma-filters"
        className="scroll-mt-32 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pb-14 sm:pb-28"
      >
        <div className="rounded-[28px] sm:rounded-[44px] bg-black text-white p-5 sm:p-9 lg:p-12">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-white/40">
              Подбор места
            </p>
            <h2 className="mt-3 sm:mt-4 text-[30px] sm:text-4xl lg:text-5xl leading-tight font-bold tracking-tight">
              Что тебе подходит сегодня?
            </h2>
            <p className="mt-3 sm:mt-4 text-white/55 text-sm sm:text-lg max-w-2xl leading-6 sm:leading-7">
              Выбери настроение, бюджет, компанию и сколько времени хочется
              провести вне дома.
            </p>
          </div>
          <div className="mt-7 sm:mt-9 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div className="mt-6 sm:mt-7 flex flex-col sm:flex-row sm:items-center gap-3">
            <button
              type="button"
              onClick={openMap}
              className="rounded-full bg-white text-black px-6 sm:px-8 py-4 font-semibold hover:scale-[1.02] transition"
            >
            ...
            <button
              type="button"
              onClick={() => {
                setMood("Любое настроение");
                setBudget("Любой");
                setCompany("Любая");
                setDuration("Любая");
              }}
              className="rounded-full border border-white/15 text-white/60 px-6 py-3.5 sm:py-4 hover:text-white hover:border-white/30 transition"
            >
              Сбросить
            </button>
          </div>
      ...
      <section
        id="alma-photozones"
        className="scroll-mt-24 bg-[#171614] text-white py-16 sm:py-28 overflow-hidden"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <h2 className="mt-4 text-[42px] sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[.96]">
                Петербург,
                <br />
            ...
            <div className="max-w-md lg:pb-1">
              <button
                ...
                onClick={() =>
                  window.alert(
                    "Приложение ALMA скоро будет доступно для скачивания.",
                  )
                }
                className="mb-7 inline-flex min-h-12 items-center justify-center rounded-full border border-white/25 bg-white px-6 py-3 text-sm font-semibold text-black transition hover:scale-[1.02] hover:bg-white/90 sm:mb-9"
              >
                Скачать приложение
              </button>
              <p className="text-sm leading-7 text-white/55 sm:text-lg">
                Красивые точки, свет и время для кадра. Листай как визуальный
                альбом и сохраняй идеи для прогулки.
              </p>
            </div>
          </div>
          <div className="mt-10 sm:mt-14 columns-2 lg:columns-4 gap-3 sm:gap-4">
            {photozones.map((place, index) => (
              <article
                key={place.title}
                className="group relative mb-3 sm:mb-4 break-inside-avoid overflow-hidden rounded-[22px] sm:rounded-[28px] bg-white/5"
              >
                <div
                  className={`${index === 0 || index === 3 ? "h-[340px] sm:h-[500px]" : "h-[260px] sm:h-[390px]"} relative`}
                >
                  <img
                    src={place.image}
                    alt={place.title}
                  ...
                  <span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1.5 text-[11px] text-black">
                    {place.tag}
                  </span>
                  <div className="absolute left-4 right-4 bottom-4">
                    <p className="text-xs text-white/55">{place.time}</p>
                    <h3 className="mt-1 text-lg sm:text-2xl font-bold">
                      {place.title}
                    </h3>
                  </div>
                </div>
          ...
          <div className="mt-8 sm:mt-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 border-t border-white/10 pt-7">
            <div>
              <p className="font-semibold">ALMA+ · подробный фотогид</p>
              <p className="mt-1 text-sm text-white/45">
                Точная точка съёмки · лучший свет · ракурс · подсказка для
                образа
              </p>
            </div>
            <button
              type="button"
              onClick={() => router.push("/surprise")}
              className="self-start rounded-full bg-white text-black px-6 py-3.5 text-sm font-semibold"
            >
              Добавить фототочки в маршрут →
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-32">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 sm:gap-5 mb-7 sm:mb-9">
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-neutral-500">
              Идеи для прогулки
            </p>
            <h2 className="mt-3 sm:mt-4 text-4xl sm:text-5xl font-bold tracking-tight">
              Популярные места
            </h2>
          </div>
          <button
            ...
            className="self-start rounded-full border border-black/10 bg-white px-5 sm:px-6 py-3 text-sm font-medium hover:bg-black hover:text-white transition"
          >
            Смотреть все →
          </button>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {places.map((place) => (
            <article
              key={place.title}
              className="group relative min-h-[340px] sm:min-h-[420px] rounded-[24px] sm:rounded-[28px] overflow-hidden bg-neutral-200"
            >
              <img
                src={place.image}
                alt={place.title}
              ...
              <div className="absolute top-4 left-4">
                <span className="inline-flex rounded-full bg-white/90 backdrop-blur px-3 py-1.5 text-xs text-black">
                  {place.category}
                </span>
              </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24">
        <div className="rounded-[28px] sm:rounded-[38px] bg-[#e9e4dc] p-6 sm:p-12 lg:p-16">
          <div className="max-w-3xl">
            <p className="text-xs uppercase tracking-[0.22em] text-neutral-500">
              ALMA
            </p>
            <h2 className="mt-4 sm:mt-5 text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.02]">
              Петербург под твоё настроение
            </h2>
            <p className="mt-4 sm:mt-5 text-base sm:text-lg text-neutral-500 leading-7 sm:leading-8 max-w-2xl">
              Не нужно заранее знать, куда именно идти. Достаточно понять, чего
              хочется сегодня.
            </p>
            <button
              type="button"
              onClick={scrollToFilters}
              className="mt-7 sm:mt-8 rounded-full bg-black text-white px-7 py-4 font-medium hover:opacity-80 transition"
            >
              Подобрать место
            </button>
          </div>
