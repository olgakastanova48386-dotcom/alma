...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f7f4ef] text-black">
      <section className="relative h-[330px] sm:h-[390px] md:h-auto md:min-h-[680px] flex items-start md:items-center pt-[76px] sm:pt-[96px] md:pt-28 pb-0 overflow-hidden bg-[#f7f4ef] text-black">
        ...
        <div className="absolute inset-x-0 bottom-3 z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-4 md:relative md:inset-auto md:bottom-auto lg:px-8"><div className="max-w-[760px]">
          ...
          <p className="mt-1.5 md:mt-7 max-w-[94%] md:max-w-xl text-[13px] md:text-xl leading-[1.35] md:leading-8 md:leading-8 text-white/82 md:text-neutral-600">ALMA помогает находить места Петербурга по настроению, бюджету, компании и времени.</p>
      ...
      <div id="alma-filters" className="relative z-10 bg-[#f7f4ef] pt-2 sm:pt-4 md:pt-0">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12 lg:py-14 text-black"><div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 sm:gap-5 mb-4 sm:mb-9"><div><h2 className="mt-3 sm:mt-4 max-w-full text-[clamp(27px,8vw,48px)] font-bold tracking-tight leading-tight text-black break-words">Популярные места</h2></div><button type="button" onClick={() => router.push("/map")} className="self-start rounded-full border border-black/10 bg-white px-5 sm:px-6 py-3 text-sm font-medium hover:bg-black hover:text-white transition">Смотреть все →</button></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">{places.map((place) => <button key={place.title} type="button" onClick={() => openPlace(place.title)} className="group relative min-h-[270px] sm:min-h-[420px] rounded-[24px] sm:rounded-[28px] overflow-hidden bg-neutral-200 text-left cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black"><img src={place.image} alt={place.title} className="absolute inset-0 w-full h-full object-cover transition duration-700 group-hover:scale-[1.04]" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/5 to-transparent" /><div className="absolute top-4 left-4"><span className="inline-flex rounded-full bg-white/90 backdrop-blur px-3 py-1.5 text-xs text-black">{place.category}</span></div><div className="absolute left-5 right-5 bottom-5"><h3 className="line-clamp-2 max-w-full text-[clamp(20px,6vw,24px)] font-bold text-white leading-tight break-normal">{place.title}</h3></div></button>)}</div></section>
