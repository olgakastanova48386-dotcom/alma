...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f7f4ef] text-black">
      <section className="relative h-[390px] sm:h-[430px] md:h-auto md:min-h-[680px] flex items-start md:items-center pt-[92px] sm:pt-[108px] md:pt-28 pb-0 overflow-hidden bg-[#f7f4ef] text-black">
        <div className="md:hidden absolute inset-0 overflow-hidden bg-[#171614]"><img key={`mobile-bg-${heroTheme.image}`} src={heroTheme.image} alt={heroTheme.label} className="absolute inset-0 h-full w-full object-cover brightness-[1.08] contrast-[0.9] saturate-[0.88] transition-opacity duration-700" style={{ objectPosition: "center 42%" }} /></div>
        ...
        <div className="absolute inset-x-0 bottom-4 z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-4 md:relative md:inset-auto md:bottom-auto lg:px-8"><div className="max-w-[760px]">
          ...
          <h1 className="mt-0 md:mt-7 max-w-full text-[clamp(27px,7.6vw,82px)] font-bold leading-[1.02] md:leading-[0.98] tracking-tight text-white md:text-black drop-shadow-[0_2px_12px_rgba(0,0,0,.28)] md:drop-shadow-none break-words">Места, в которые<br />хочется вернуться</h1>
          <p className="mt-2 md:mt-7 max-w-[94%] md:max-w-xl text-[13px] md:text-xl leading-[1.35] md:leading-8 md:leading-8 text-white/82 md:text-neutral-600">ALMA помогает находить места Петербурга по настроению, бюджету, компании и времени.</p>
          <div className="mt-3 md:mt-9 grid grid-cols-3 gap-1.5 md:flex md:flex-wrap md:gap-3">
      ...
      <div id="alma-filters" className="relative z-10 bg-[#f7f4ef] pt-4 sm:pt-5 md:pt-0">
      ...
      <section id="alma-photozones" className="scroll-mt-24 bg-[#171614] text-white pt-6 pb-7 sm:pt-8 sm:pb-9 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center text-center">
            <h2 className="max-w-full text-[clamp(34px,7vw,64px)] font-bold tracking-tight leading-[.96] break-words">Фотолокации</h2>
            <button type="button" onClick={() => router.push("/photozones/add")} className="mt-5 inline-flex min-h-11 items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-semibold text-black transition hover:bg-[#f4d37c]">＋ Добавить фотолокацию</button>
          </div>
          <div className="mt-7 sm:mt-9 columns-2 lg:columns-4 gap-3 sm:gap-4">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-9 sm:py-12 lg:py-14 text-black"><div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 sm:gap-5 mb-7 sm:mb-9"><div><h2 className="mt-3 sm:mt-4 max-w-full text-[clamp(30px,8vw,48px)] font-bold tracking-tight leading-tight text-black break-words">Популярные места</h2></div><button type="button" onClick={() => router.push("/map")} className="self-start rounded-full border border-black/10 bg-white px-5 sm:px-6 py-3 text-sm font-medium hover:bg-black hover:text-white transition">Смотреть все →</button></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">{places.map((place) => <button key={place.title} type="button" onClick={() => openPlace(place.title)} className="group relative min-h-[340px] sm:min-h-[420px] rounded-[24px] sm:rounded-[28px] overflow-hidden bg-neutral-200 text-left cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black"><img src={place.image} alt={place.title} className="absolute inset-0 w-full h-full object-cover transition duration-700 group-hover:scale-[1.04]" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/5 to-transparent" /><div className="absolute top-4 left-4"><span className="inline-flex rounded-full bg-white/90 backdrop-blur px-3 py-1.5 text-xs text-black">{place.category}</span></div><div className="absolute left-5 right-5 bottom-5"><h3 className="line-clamp-2 max-w-full text-[clamp(20px,6vw,24px)] font-bold text-white leading-tight break-normal">{place.title}</h3></div></button>)}</div></section>

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 sm:pb-16 text-black"><div className="overflow-hidden rounded-[22px] sm:rounded-[28px] bg-[#e9e4dc] p-5 sm:p-8 lg:p-10 text-black"><div className="max-w-3xl"><p className="text-xs uppercase tracking-[0.22em] text-neutral-500">ALMA</p><h2 className="mt-3 sm:mt-4 max-w-full text-[clamp(25px,6vw,44px)] font-bold tracking-tight leading-[1.04] text-black break-words [overflow-wrap:anywhere]">Петербург под твоё настроение</h2><p className="mt-3 sm:mt-4 text-sm sm:text-base text-neutral-600 leading-6 sm:leading-7 max-w-2xl">Не нужно заранее знать, куда именно идти. Достаточно понять, чего хочется сегодня.</p><button type="button" onClick={scrollToFilters} className="mt-5 sm:mt-6 rounded-full bg-black text-white px-6 py-3 font-medium hover:opacity-80 transition">Подобрать место</button></div></div></section>
    </main>
  );
}
