  ...
  return <main className="min-h-screen bg-[#f7f4ef]">
    <section className="relative min-h-0 md:min-h-[820px] flex items-center pt-24 sm:pt-28 pb-10 sm:pb-20 overflow-hidden bg-[#efe9e1] text-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8"><div className="max-w-[760px]">
        ...
        <div className="mt-7 sm:mt-9 grid grid-cols-2 sm:flex sm:flex-wrap gap-2.5 sm:gap-3"><button type="button" onClick={scrollToFilters} className="rounded-full bg-black text-white px-4 sm:px-7 py-3.5 sm:py-4 text-sm sm:text-base font-semibold hover:opacity-80 hover:scale-[1.02] transition">Найти место</button><button type="button" onClick={() => router.push("/surprise")} className="rounded-full bg-white/80 backdrop-blur-md border border-black/10 text-black px-4 sm:px-7 py-3.5 sm:py-4 text-sm sm:text-base font-medium hover:bg-white hover:scale-[1.02] transition shadow-sm">✦ Удиви меня</button></div>
        ...
        <div className="md:hidden mt-8 relative h-[430px] w-full overflow-hidden rounded-[28px] shadow-[0_24px_55px_rgba(0,0,0,.18)] ring-1 ring-black/5 bg-neutral-200">
          <img key={`mobile-${heroTheme.image}`} src={heroTheme.image} alt={heroTheme.label} className="h-full w-full object-cover" style={{ objectPosition: heroTheme.position }} />
          <div className="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-white/5" />
          <div className="absolute left-4 bottom-4 rounded-full bg-black/70 backdrop-blur-md px-3 py-2 text-[11px] uppercase tracking-[0.14em] text-white/80">{heroTheme.label}</div>
        </div>
      </div></div>
