  ...
  return <main className="min-h-screen bg-[#f7f4ef]">
    <section className="relative min-h-0 md:min-h-[820px] flex items-center pt-24 sm:pt-28 pb-10 sm:pb-20 overflow-hidden bg-[#efe9e1] text-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8"><div className="max-w-[760px]">
        ...
        <div className="mt-7 sm:mt-9 grid grid-cols-2 sm:flex sm:flex-wrap gap-2.5 sm:gap-3"><button type="button" onClick={scrollToFilters} className="rounded-full bg-black text-white px-4 sm:px-7 py-3.5 sm:py-4 text-sm sm:text-base font-semibold hover:opacity-80 hover:scale-[1.02] transition">Найти место</button><button type="button" onClick={() => router.push("/surprise")} className="rounded-full bg-white/80 backdrop-blur-md border border-black/10 text-black px-4 sm:px-7 py-3.5 sm:py-4 text-sm sm:text-base font-medium hover:bg-white hover:scale-[1.02] transition shadow-sm">✦ Удиви меня</button><button type="button" onClick={() => router.push("/photozones")} className="rounded-full bg-white/80 backdrop-blur-md border border-black/10 text-black px-4 sm:px-7 py-3.5 sm:py-4 text-sm sm:text-base font-medium hover:bg-white hover:scale-[1.02] transition shadow-sm">📸 Фотозоны</button></div>
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-28"><div className="grid lg:grid-cols-[.9fr_1.1fr] gap-6 items-stretch"><div className="rounded-[28px] sm:rounded-[38px] bg-[#e9e4dc] p-6 sm:p-10"><p className="text-xs uppercase tracking-[.22em] text-neutral-500">📸 ALMA · Фотозоны</p><h2 className="mt-4 text-4xl sm:text-5xl font-bold tracking-tight">Красивые точки по пути</h2><p className="mt-4 max-w-xl text-base sm:text-lg leading-7 text-neutral-500">Смотри фотозоны бесплатно, а в полном фотогиде получай точку съёмки, лучшее время, ракурс и подсказки для кадра.</p><button type="button" onClick={() => router.push("/photozones")} className="mt-7 rounded-full bg-black text-white px-6 py-3.5 font-medium">Открыть фотозоны →</button></div><div className="grid grid-cols-2 gap-3"><div className="relative min-h-[250px] sm:min-h-[360px] overflow-hidden rounded-[24px] sm:rounded-[30px]"><img src="/images/kisses-bridge.jpg" alt="Поцелуев мост" className="absolute inset-0 h-full w-full object-cover"/></div><div className="relative min-h-[250px] sm:min-h-[360px] overflow-hidden rounded-[24px] sm:rounded-[30px]"><img src="/images/isaac.jpg" alt="Исаакиевская площадь" className="absolute inset-0 h-full w-full object-cover"/></div></div></div></section>

    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-32"><div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 sm:gap-5 mb-7 sm:mb-9"><div><p className="text-xs uppercase tracking-[0.22em] text-neutral-500">Идеи для прогулки</p><h2 className="mt-3 sm:mt-4 text-4xl sm:text-5xl font-bold tracking-tight">Популярные места</h2></div><button type="button" onClick={() => router.push("/map")} className="self-start rounded-full border border-black/10 bg-white px-5 sm:px-6 py-3 text-sm font-medium hover:bg-black hover:text-white transition">Смотреть все →</button></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">{places.map(place => <article key={place.title} className="group relative min-h-[340px] sm:min-h-[420px] rounded-[24px] sm:rounded-[28px] overflow-hidden bg-neutral-200"><img src={place.image} alt={place.title} className="absolute inset-0 w-full h-full object-cover transition duration-700 group-hover:scale-[1.04]" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/5 to-transparent" /><div className="absolute top-4 left-4"><span className="inline-flex rounded-full bg-white/90 backdrop-blur px-3 py-1.5 text-xs text-black">{place.category}</span></div><div className="absolute left-5 right-5 bottom-5"><h3 className="text-2xl font-bold text-white leading-tight">{place.title}</h3></div></article>)}</div></section>
