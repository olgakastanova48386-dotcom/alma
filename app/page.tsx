      ...
      <section className="alma-home-hero relative h-[390px] sm:h-[430px] md:h-auto md:min-h-[680px] flex items-start md:items-center pt-0 md:pt-28 pb-0 overflow-hidden bg-[#f7f4ef] text-black">
        ...
        <div className="absolute inset-x-0 bottom-3 z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-4 md:relative md:inset-auto md:bottom-auto lg:px-8"><div className="max-w-[760px]">
          ...
          <div className="mt-2.5 md:mt-9 grid grid-cols-3 gap-1.5 md:flex md:flex-wrap md:gap-3">
            <button type="button" onClick={scrollToFilters} className="min-w-0 overflow-hidden whitespace-nowrap rounded-full bg-white text-black md:bg-black md:text-white px-1.5 md:px-7 py-2.5 md:py-4 text-[12px] md:text-base font-semibold leading-none hover:opacity-80 hover:scale-[1.02] transition">Найти место</button>
            ...
            <button type="button" onClick={scrollToPhotozones} className="min-w-0 overflow-hidden whitespace-nowrap rounded-full bg-black/55 md:bg-white/80 backdrop-blur-md border border-white/25 md:border-black/10 text-white md:text-black px-1.5 md:px-7 py-2.5 md:py-4 text-[12px] md:text-base font-medium leading-none hover:scale-[1.02] transition shadow-sm">📸 Фотозоны</button>
          </div>
        </div></div>
      ...
      <section id="alma-photozones" className="scroll-mt-24 bg-[#171614] text-white pt-4 pb-5 sm:pt-8 sm:pb-9 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 sm:mt-9 columns-2 lg:columns-4 gap-3 sm:gap-4">
            {photozones.map((place, index) => <button key={place.title} type="button" onClick={() => openPlace(place.title)} aria-label={`Открыть ${place.title}`} className="group relative mb-3 sm:mb-4 block w-full break-inside-avoid overflow-hidden rounded-[22px] sm:rounded-[28px] bg-white/5 text-left cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70"><div className={`${index === 0 || index === 3 ? "h-[190px] sm:h-[500px]" : "h-[160px] sm:h-[390px]"} relative`}><img src={place.image} alt={place.title} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/5" /><span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1.5 text-[11px] text-black">{place.tag}</span><div className="absolute left-3 right-3 bottom-4 sm:left-4 sm:right-4"><p className="text-[11px] sm:text-xs text-white/55 line-clamp-1">{place.time}</p><h3 className="mt-1 line-clamp-2 min-h-[2.2em] max-w-full text-[16px] sm:text-[22px] font-bold leading-[1.08] text-white break-normal">{place.title}</h3></div></div></button>)}
          </div>
        </div>
      </section>

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-12 lg:py-14 text-black"><div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 sm:gap-5 mb-3 sm:mb-9"><div><h2 className="mt-0 sm:mt-4 max-w-full text-[clamp(25px,7vw,48px)] font-bold tracking-tight leading-tight text-black break-words">Популярные места</h2></div><button type="button" onClick={scrollToFilters} className="self-start rounded-full border border-black/10 bg-white px-5 sm:px-6 py-3 text-sm font-medium hover:bg-black hover:text-white transition">Смотреть все →</button></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">{places.map((place) => <button key={place.title} type="button" onClick={() => openPlace(place.title)} className="group relative min-h-[190px] sm:min-h-[420px] rounded-[24px] sm:rounded-[28px] overflow-hidden bg-neutral-200 text-left cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black"><img src={place.image} alt={place.title} className={`absolute inset-0 w-full h-full object-cover transition duration-700 group-hover:scale-[1.04] ${place.title === "Петропавловская крепость" ? "object-top" : "object-center"}`} /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/5 to-transparent" /><div className="absolute top-4 left-4"><span className="inline-flex rounded-full bg-white/90 backdrop-blur px-3 py-1.5 text-xs text-black">{place.category}</span></div><div className="absolute left-5 right-5 bottom-5"><h3 className="line-clamp-2 max-w-full text-[clamp(20px,6vw,24px)] font-bold text-white leading-tight break-normal">{place.title}</h3></div></button>)}</div></section>
