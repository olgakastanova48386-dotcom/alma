...
export default function PlacePage() {
  ...
  const isFoodVenue = ["Кофейня", "Ресторан", "Кафе", "Кафе-ресторан"].includes(place.category);
        ...
        <section className="grid lg:grid-cols-[1.08fr_0.92fr] gap-5 sm:gap-8 lg:gap-12 items-stretch">
          ...
          <div className="flex flex-col justify-between py-1 lg:py-3">
            <div>
              ...
              <div className="mt-3 sm:mt-4 rounded-[18px] sm:rounded-[24px] bg-white border border-black/5 p-4 sm:p-6"><p className="text-xs sm:text-sm text-neutral-500">Адрес</p><div className="mt-2 flex items-start gap-2 sm:gap-3"><span className="text-base sm:text-lg">📍</span><p className="text-sm sm:text-lg font-medium leading-6 sm:leading-7">{place.address}</p></div>{place.dogFriendly && <div className="mt-3 rounded-xl sm:rounded-2xl bg-[#eef3e9] px-3 py-2 text-xs sm:px-4 sm:py-3 sm:text-sm font-medium">🐾 Dog Friendly</div>}</div>

              {isFoodVenue && <div className="mt-3 sm:mt-4 rounded-[18px] sm:rounded-[24px] bg-white border border-black/5 p-4 sm:p-6"><p className="text-xs sm:text-sm text-neutral-500">Условия в заведении</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-2xl bg-[#eef3e9] px-4 py-3"><div className="flex items-center gap-2"><span className="text-xl" aria-hidden="true">🚭</span><p className="text-sm font-semibold">В помещении не курят</p></div><p className="mt-1 text-xs leading-5 text-neutral-500">Табак, вейпы и кальяны запрещены.</p></div><div className={`rounded-2xl px-4 py-3 ${place.babyCare ? "bg-[#efe5d7]" : "bg-neutral-100"}`}><div className="flex items-center gap-2"><span className={`text-xl ${place.babyCare ? "" : "grayscale opacity-45"}`} aria-hidden="true">👶</span><p className={`text-sm font-semibold ${place.babyCare ? "text-black" : "text-neutral-500"}`}>{place.babyCare ?? "Не подтверждено"}</p></div><p className="mt-1 text-xs leading-5 text-neutral-500">{place.babyCare ? "Есть удобство для малыша." : "Заведение пока не сообщило об удобствах для малышей."}</p></div></div>{place.menuUrl ? <a href={place.menuUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-black px-5 py-3 text-sm font-medium text-white transition hover:opacity-80">Посмотреть меню ↗</a> : <div className="mt-3 rounded-2xl bg-neutral-100 px-4 py-3 text-center text-sm text-neutral-500">Меню пока не добавлено</div>}</div>}
