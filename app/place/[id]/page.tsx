...
export default function PlacePage() {
    ...
    async function loadFavorites() {
      ...
      try {
        ...
        const ids = Array.isArray(data.ids)
          ? data.ids.map(Number).filter(Number.isFinite)
          : localIds;
  ...
  const gallery = place?.gallery?.length
    ? place.gallery
    : place?.image
      ? [place.image]
      : [];
  ...
  const isFoodVenue = ["Кофейня", "Ресторан", "Кафе", "Кафе-ресторан"].includes(
    place.category,
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="grid lg:grid-cols-[1.08fr_0.92fr] gap-5 sm:gap-8 lg:gap-12 items-stretch">
          <div>
            <div className="relative min-h-[340px] sm:min-h-[560px] lg:min-h-[700px] rounded-[22px] sm:rounded-[40px] overflow-hidden bg-neutral-200">
              ...
              <div className="absolute top-4 left-4 sm:top-7 sm:left-7">
                <span className="inline-flex rounded-full bg-white/90 backdrop-blur-md px-3 py-1.5 text-xs sm:px-4 sm:py-2 sm:text-sm text-black shadow-sm">
                  {place.category}
                </span>
              </div>
              <div className="absolute left-4 right-4 bottom-4 sm:left-7 sm:right-7 sm:bottom-7">
                <div className="inline-flex rounded-full bg-black/80 backdrop-blur-md text-white px-3 py-1.5 text-xs sm:px-4 sm:py-2 sm:text-sm">
                  ✦ {place.mood}
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col justify-between py-1 lg:py-3">
            <div>
              ...
              {place.rating && (
                <div className="mt-3 sm:mt-5 flex flex-wrap items-center gap-2 sm:gap-3">
                  <span className="rounded-full bg-black text-white px-3 py-1.5 text-xs sm:px-4 sm:py-2 sm:text-sm font-semibold">
                    ★ {place.rating.toFixed(1)} / 5
                  </span>
                  {place.ratingSource && (
                    <span className="text-xs sm:text-sm text-neutral-500">
                      Источник: {place.ratingSource}
                    </span>
                  )}
                </div>
              )}
              ...
              {place.description && (
                <div className="mt-5 sm:mt-7 rounded-[18px] sm:rounded-[28px] bg-white border border-black/5 p-4 sm:p-7">
                  ...
                  <p className="mt-2 sm:mt-3 text-[15px] sm:text-lg leading-6 sm:leading-8 text-neutral-700">
                    {place.description}
                  </p>
                </div>
              )}
              ...
              <div className="mt-3 sm:mt-6 rounded-[18px] sm:rounded-[28px] bg-white border border-black/5 p-4 sm:p-7">
                ...
                {!isFoodVenue && (
                  <p className="mt-1 text-sm sm:mt-2 sm:text-base font-medium text-neutral-700">
                    {place.priceNote}
                  </p>
                )}
              </div>
              ...
              <div className="mt-3 sm:mt-4 grid grid-cols-3 gap-2 sm:gap-3">
                <div className="rounded-[16px] sm:rounded-[24px] bg-white border border-black/5 p-3 sm:p-5">
                  ...
                  <p className="mt-1 text-[13px] sm:mt-2 sm:text-lg font-semibold leading-4 sm:leading-normal">
                    {place.mood}
                  </p>
                </div>
                <div className="rounded-[16px] sm:rounded-[24px] bg-white border border-black/5 p-3 sm:p-5">
                ...
                <div className="rounded-[16px] sm:rounded-[24px] bg-white border border-black/5 p-3 sm:p-5">
                  ...
                  <p className="mt-1 text-[13px] sm:mt-2 sm:text-lg font-semibold leading-4 sm:leading-7">
                    {place.company.join(", ")}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-3 sm:mt-4 rounded-[18px] sm:rounded-[24px] bg-white border border-black/5 p-4 sm:p-6">
                ...
                <div className="mt-2 flex items-start gap-2 sm:gap-3">
                  ...
                  <p className="text-sm sm:text-lg font-medium leading-6 sm:leading-7">
                    {place.address}
                  </p>
                </div>
                {place.dogFriendly && (
                  <div className="mt-3 rounded-xl sm:rounded-2xl bg-[#eef3e9] px-3 py-2 text-xs sm:px-4 sm:py-3 sm:text-sm font-medium">
                    🐾 Dog Friendly
                  </div>
                )}
              ...
              {isFoodVenue && (
                <div className="mt-3 sm:mt-4 rounded-[18px] sm:rounded-[24px] bg-white border border-black/5 p-4 sm:p-6">
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <div className="rounded-2xl bg-[#eef3e9] px-4 py-3">
                      ...
                      <p className="mt-1 text-xs leading-5 text-neutral-500">
                        Табак, вейпы и кальяны запрещены.
                      </p>
                    </div>
                    <div
                      className={`rounded-2xl px-4 py-3 ${place.babyCare ? "bg-[#efe5d7]" : "bg-neutral-100"}`}
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-xl ${place.babyCare ? "" : "grayscale opacity-45"}`}
                          aria-hidden="true"
                        >
                          👶
                        </span>
                        <p
                          className={`text-sm font-semibold ${place.babyCare ? "text-black" : "text-neutral-500"}`}
                        >
                          {place.babyCare ?? "Не подтверждено"}
                        </p>
                      </div>
                      <p className="mt-1 text-xs leading-5 text-neutral-500">
                        {place.babyCare
                          ? "Есть удобство для малыша."
                          : "Заведение пока не сообщило об удобствах для малышей."}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {favoritesLoaded && !signedIn && (
              <p className="mt-3 text-center sm:text-left text-xs text-neutral-400">
                Войдите в ALMA, чтобы избранное сохранилось в аккаунте.
              </p>
            )}
          </div>
        </section>
