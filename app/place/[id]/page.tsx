        ...
        <section className="grid lg:grid-cols-[1.08fr_0.92fr] gap-8 lg:gap-12 items-stretch">
          ...
          <div className="flex flex-col justify-between py-1 lg:py-3">
            <div>
              ...
              {place.rating && <div className="mt-5 flex flex-wrap items-center gap-3"><span className="rounded-full bg-black text-white px-4 py-2 text-sm font-semibold">★ {place.rating.toFixed(1)} / 5</span>{place.ratingSource && <span className="text-sm text-neutral-500">Источник рейтинга: {place.ratingSource}</span>}</div>}
              ...
              <div className="mt-6 rounded-[28px] bg-white border border-black/5 p-6 sm:p-7"><p className="text-sm text-neutral-500">Стоимость</p><p className="mt-2 text-3xl sm:text-4xl font-bold tracking-tight text-black">{place.price}</p><p className="mt-2 text-base font-medium text-neutral-700">{place.priceNote}</p></div>
              <div className="mt-4 grid sm:grid-cols-3 gap-3"><div className="rounded-[24px] bg-white border border-black/5 p-5"><p className="text-sm text-neutral-500">Настроение</p><p className="mt-2 text-lg font-semibold">{place.mood}</p></div><div className="rounded-[24px] bg-white border border-black/5 p-5"><p className="text-sm text-neutral-500">Длительность</p><p className="mt-2 text-lg font-semibold">{place.duration}</p></div><div className="rounded-[24px] bg-white border border-black/5 p-5"><p className="text-sm text-neutral-500">Компания</p><p className="mt-2 text-base sm:text-lg font-semibold leading-7">{place.company.join(", ")}</p></div></div>
              <div className="mt-4 rounded-[24px] bg-white border border-black/5 p-5 sm:p-6"><p className="text-sm text-neutral-500">Адрес</p><div className="mt-2 flex items-start gap-3"><span className="text-lg">📍</span><p className="text-base sm:text-lg font-medium leading-7">{place.address}</p></div>{place.babyCare && <div className="mt-4 rounded-2xl bg-[#efe5d7] px-4 py-3 text-sm font-medium">👶 {place.babyCare}</div>}{place.dogFriendly && <div className="mt-3 rounded-2xl bg-[#eef3e9] px-4 py-3 text-sm font-medium">🐾 Dog Friendly</div>}</div>
              <PlaceOutfitAdvice lat={place.lat} lng={place.lng} category={place.category} />
            </div>

            <div className="mt-8 flex flex-col sm:flex-row gap-3"><Link href={`/map?place=${place.id}`} className="flex-1 rounded-full bg-black text-white px-7 py-4 text-center font-medium hover:opacity-85 transition">Показать на карте</Link><button type="button" onClick={toggleFavorite} disabled={!favoritesLoaded} aria-pressed={isFavorite} className={`flex-1 rounded-full border px-7 py-4 font-medium transition-all duration-300 ${isFavorite ? "bg-black border-black text-white" : "bg-white border-black/10 text-black hover:bg-black hover:text-white"}`}>{isFavorite ? "♥ В избранном" : "♡ В избранное"}</button></div>
          </div>
        </section>
      </div>
    </main>
