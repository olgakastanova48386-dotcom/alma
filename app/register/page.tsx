    ...
    <main className="min-h-screen bg-[#f7f4ef] flex items-center justify-center px-4 pt-24 pb-12">
      <div className="bg-white p-8 sm:p-10 rounded-[30px] shadow-xl w-full max-w-[430px] border border-black/5">
        ...
        <form className="mt-7">
          ...
          {name && !nameOk && <p className="mt-2 text-xs text-red-600">Введите имя буквами, без цифр и случайного набора символов.</p>}
          ...
          <fieldset className="mt-5">
            ...
            <p className="mt-1 text-xs leading-5 text-neutral-400">Используем только для функций, где это действительно нужно — например, чтобы предложить дополнительный режим маршрута.</p>
            <div className="mt-3 grid grid-cols-3 gap-2">
              {([{value:"female",label:"Женщина"},{value:"male",label:"Мужчина"},{value:"unspecified",label:"Не указывать"}] as const).map((option)=><button key={option.value} type="button" onClick={()=>setGender(option.value)} aria-pressed={gender===option.value} className={`rounded-2xl border px-2 py-3 text-xs font-medium transition ${gender===option.value?"border-black bg-black text-white":"border-black/10 bg-white text-neutral-600 hover:border-black/30"}`}>{option.label}</button>)}
            </div>
            <input type="hidden" name="gender" value={gender}/>
          </fieldset>

          <input type="password" name="password" autoComplete="new-password" minLength={8} placeholder="Пароль · минимум 8 символов" className="mt-5 w-full border border-black/30 rounded-2xl px-4 py-3.5 outline-none focus:border-black transition" />
        ...
        <p className="mt-4 text-center text-xs leading-5 text-neutral-400">Создание аккаунта включим вместе с защищённым хранением и подтверждением телефона.</p>
      </div>
    </main>
  );
}
