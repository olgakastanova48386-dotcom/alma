        ...
        <div className="mt-10 grid md:grid-cols-2 xl:grid-cols-3 gap-5">
          {restaurantPlaces.map((place, index) => (
            <article key={place.id} className="rounded-[28px] bg-white border border-black/5 p-6 shadow-sm flex flex-col">
              ...
              {place.babyCare && <div className="mt-4"><span className="inline-flex rounded-full bg-[#efe5d7] px-3 py-2 text-xs font-semibold text-neutral-800">👶 {place.babyCare}</span></div>}
              <div className="mt-5 space-y-2 text-sm text-neutral-600"><p><span className="text-black font-medium">Адрес:</span> {place.address}</p>{place.averageBill && <p><span className="text-black font-medium">Средний счёт:</span> {place.averageBill}</p>}{place.phone && <p><span className="text-black font-medium">Телефон:</span> {place.phone}</p>}{place.dogFriendly && <p><span className="text-black font-medium">Dog Friendly:</span> {place.dogFriendly}</p>}{place.babyCare && <p><span className="text-black font-medium">Для малыша:</span> {place.babyCare}{place.babyCareVerifiedAt ? ` · проверено ${place.babyCareVerifiedAt}` : ""}</p>}</div>
