  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) setFavoriteIds(parsed.map(Number).filter(Number.isFinite));
      }
    } catch { setFavoriteIds([]); } finally { setLoaded(true); }
  }, []);
