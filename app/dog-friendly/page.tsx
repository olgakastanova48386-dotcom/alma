    ...
    <main className="min-h-screen bg-[#f7f4ef] pt-32 sm:pt-36 pb-24 text-black">
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="rounded-[36px] bg-[#dfe8d8] p-7 sm:p-10 lg:p-14 overflow-hidden relative">
          ...
          <p className="mt-6 max-w-2xl text-base sm:text-lg text-neutral-700 leading-7">Места, куда можно прийти вместе с питомцем. Показываем рейтинг с источником, актуальные условия для собак и фотографии конкретных заведений.</p>
        </div>
          ...
          {filtered.map((place) => (
            <article key={place.id} className="overflow-hidden rounded-[30px] bg-white border border-black/5 shadow-sm">
              ...
              <div className="p-6 sm:p-7">
                ...
                {place.rating && place.ratingScale && (
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-black text-white px-3 py-2 text-sm font-semibold">★ {place.rating.toFixed(1)} / {place.ratingScale}</span>
                    <span className="text-xs text-neutral-500">Рейтинг · {place.ratingSource}{place.ratingCount ? ` · ${place.ratingCount.toLocaleString("ru-RU")} оценок` : ""}</span>
                  </div>
                )}
        ...
        <div className="mt-10 rounded-[28px] bg-black text-white p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-4xl text-sm sm:text-base leading-7 text-white/70">ALMA всегда показывает источник и шкалу рейтинга: оценки разных сервисов не смешиваем. Условия для собак берём из опубликованных данных заведения или профильных источников и не придумываем ограничения, которых источник не указывает.</p>
        </div>
      </section>
    </main>
  );
