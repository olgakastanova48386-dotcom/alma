...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ total:Number(s?.total||0), today:Number(s?.today||0), week:Number(s?.week||0), month:Number(s?.month||0), lastRegistration:s?.last_registration?Number(s.last_registration):null, pageViews:{total:Number(views?.total||0),today:Number(views?.today||0),week:Number(views?.week||0),month:Number(views?.month||0)}, pages:(pages?.results||[]).map((row:any)=>({path:String(row.path),views:Number(row.views||0)})) });
