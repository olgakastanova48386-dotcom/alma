...
import { authDb, consumeAuthRateLimit, createSession, ensureAuthSchema, hashPassword, isValidEmail, normalizeEmail, sessionCookie } from "@/lib/auth";
...
export async function POST(request: Request) {
  try {
    ...
    const response = NextResponse.json({ ok: true, requiresVerification: false, user: { id: userId, name, email, gender } }, { status: 201 });
