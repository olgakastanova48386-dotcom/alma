...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || !Number.isFinite(placeId) || !placeName) return NextResponse.json({ error: "Не удалось прочитать фотографию." }, { status: 400 });
    ...
    await authDb().prepare(`INSERT INTO route_photos (id, user_id, place_id, place_name, mime_type, image_base64, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)`)
...
export async function GET(request: Request) {
  try {
    ...
    if (moderation) {
      ...
      const rows = await authDb().prepare(`SELECT id, place_id, place_name, mime_type, status, created_at FROM route_photos WHERE status = 'pending' ORDER BY created_at ASC LIMIT 100`).all();
    ...
    const rows = await authDb().prepare(`SELECT id, place_id, place_name, created_at FROM route_photos WHERE place_id = ? AND status = 'approved' ORDER BY reviewed_at DESC LIMIT 8`).bind(placeId).all();
...
export async function PATCH(request: Request) {
  try {
    ...
    const now = Math.floor(Date.now() / 1000);
    await authDb().prepare(`UPDATE route_photos SET status = ?, reviewed_at = ?, reviewed_by = ? WHERE id = ? AND status = 'pending'`).bind(status, now, user.id, id).run();
