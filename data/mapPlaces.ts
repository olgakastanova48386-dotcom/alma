...
const restaurantMapPlaces: MapPlace[] = restaurantPlaces.filter((p)=>hasAlmaRating(p.rating)).map((p,index)=>{const id=2001+index;return { id,name:p.name,category:"Ресторан",mood:"Вкусно поесть",budget:p.averageBill??"1500–5000 ₽",company:["Пара","Друзья","Семья"],duration:"1–2 часа",image:p.image??"",lat:p.lat,lng:p.lng,why:`★ ${p.rating.toFixed(1)} / 5`,address:p.address,price:p.averageBill??"Чек уточняется",priceNote:"Средний чек",detailHref:`/place/${id}`,rating:p.rating,ratingScale:5,ratingCount:p.ratingCount,ratingSource:p.ratingSource,dogFriendly:Boolean(p.dogFriendly),babyCare:p.babyCare,babyCareVerifiedAt:p.babyCareVerifiedAt,menuUrl:p.website??p.sourceUrl }});

const dogMapPlaces: MapPlace[] = dogFriendlyPlaces.filter((p)=>hasAlmaRating(p.rating,p.ratingScale)).filter((d)=>![...basePlaces,...coffeeMapPlaces,...restaurantMapPlaces].some((p)=>p.name.toLowerCase()===d.name.toLowerCase())).map((p,index)=>{const id=3001+index;return { id,name:p.name,category:p.category,mood:"С питомцем",budget:p.budget,company:["Один","Пара","Друзья","Семья"],duration:"1–2 часа",image:p.imageUrl??"",lat:p.lat,lng:p.lng,why:`🐾 Dog Friendly · ★ ${p.rating.toFixed(1)} / 5`,address:p.address,price:p.budget,priceNote:"Условия посещения",detailHref:`/place/${id}`,rating:p.rating,ratingScale:5,ratingCount:p.ratingCount,ratingSource:p.ratingSource,dogFriendly:true,babyCare:p.babyCare,menuUrl:p.category==="Городское пространство"?undefined:p.sourceUrl }});
